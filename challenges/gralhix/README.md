# Gralhix OSINT Exercises

Sofia Santos (@Gralhix) が提供する無料のOSINT練習問題集。

- **URL**: https://gralhix.com/list-of-osint-exercises/
- **問題数**: 32問
- **難易度**: Easy / Medium / Hard
- **特徴**: 各問題にYouTube解説動画あり

## 難易度別一覧

| 難易度 | 問題番号 |
|--------|----------|
| Easy | 006, 021, 023, 027, 031 |
| Easy~Medium | 004, 008, 018, 028 |
| Medium | 003, 013, 017, 020, 025 |

## 解答済み

| # | タイトル | 難易度 | カテゴリ |
|---|----------|--------|----------|
| 003 | トルコ大統領府特定 | Medium | GEOINT/Building ID |
| 006 | 画像検証（TTPパキスタン攻撃） | Easy | Verification |
| 017 | カレンダー変換 | Medium | Calendar Conversion |
| 021 | チョコレート地図 | Easy | GEOINT |
| 027 | Lectura en Movimiento | Easy | Event/Person ID |
| 031 | キリン追跡（Willow） | Easy | Animal Tracking |
